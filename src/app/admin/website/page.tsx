            ...
            <div className="space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider">
                  Google Maps Embed URL / Kode Sematkan Peta
                </label>
                <button
                  type="button"
                  onClick={() => {
                    ...
                    if (!query) {
                      toast.error('Mohon isi alamat kantor atau kota terlebih dahulu');
                      return;
                    }
                    const generatedUrl = `https://maps.google.com/maps?q=${encodeURIComponent(query)}&t=&z=15&ie=UTF8&iwloc=&output=embed`;
                    setForm({ ...form, mapsEmbedUrl: generatedUrl });
                    toast.success('Peta Google Maps berhasil dibuat otomatis sesuai alamat kantor!');
                  }}
                  className="px-3 py-1.5 text-xs font-semibold bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-xl transition-colors inline-flex items-center gap-1.5 cursor-pointer self-start sm:self-auto"
                >
                  <MapPin className="w-3.5 h-3.5" />
                  <span>Generate Otomatis dari Alamat</span>
                </button>
              </div>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder='Tempel URL embed atau kode lengkap <iframe src="https://www.google.com/maps/embed?..." ...></iframe>'
              ...
              <div className="p-3 bg-amber-50/80 rounded-xl border border-amber-200/80 text-xs text-amber-900 space-y-1.5">
                <p className="font-semibold flex items-center gap-1.5 text-amber-800">
                  <span>📍</span> Cara Mendapatkan Kode Peta dari Google Maps:
                </p>
                <ol className="list-decimal list-inside space-y-1 text-amber-800 text-[11px] pl-1 leading-relaxed">
                  <li>Buka <a href="https://www.google.com/maps" target="_blank" rel="noreferrer" className="underline font-bold text-blue-700">Google Maps</a> dan cari lokasi kantor BUMDES Anda.</li>
                  <li>Klik tombol <strong>Bagikan (Share)</strong>, lalu pilih tab <strong>Sematkan peta (Embed a map)</strong>.</li>
                  <li>Klik tombol <strong>Salin HTML (Copy HTML)</strong>.</li>
                  <li>Tempel (Paste) langsung ke kolom input di atas. Sistem kami akan otomatis mengekstrak tautan peta yang benar.</li>
                  <li>Atau, klik tombol <strong>"Generate Otomatis dari Alamat"</strong> di atas jika Anda ingin peta langsung disesuaikan dengan teks Alamat Kantor & Kota.</li>
                </ol>
              </div>

              {/* Map Live Preview */}
              <div className="mt-4 rounded-2xl border border-slate-200 overflow-hidden shadow-xs bg-white">
                <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                    <MapPin className="w-3.5 h-3.5 text-blue-600" />
                    Preview Tampilan Peta
                  </span>
                  <span className="text-[11px] text-slate-400">
                    Ini tampilan yang akan dilihat pengunjung di halaman Kontak
                  </span>
                </div>
                <div className="w-full h-64 bg-slate-100 relative">
                  <iframe
                    src={getValidMapsUrl(form.mapsEmbedUrl, form.address, form.city)}
                    width="100%"
                    height="100%"
                    style={{ border: 0 }}
